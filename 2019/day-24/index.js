...
const getMapKey = (x, y, z) => `${x},${y},${z}`;
const getMapCoordinate = (key) => key.split(',').map(Number);
let map = fs_1.default.readFileSync('day-24/input.txt').toString().split('\n').flatMap((row, rowIndex) => row.split('').map((column, columnIndex) => ({
    x: columnIndex,
    y: rowIndex,
    bug: column === '#'
}))).reduce((acc, tile) => {
...
const getbBiodiversityRating = (map) => {
    let tilePoint = 1;
    let sum = 0;
...
const setMapAtLevel = (map, refMap, level) => {
    for (let x = 0; x < 5; x++) {
        for (let y = 0; y < 5; y++) {
...
const pixels = Object.entries(Array(...map.entries()).reduce((acc, [key, bug]) => {
    ...
    acc[pos[2]].push(toHtml_1.toPixel(pos, bug));
    return acc;
}, {})).sort((a, b) => Number(a[0]) - Number([0])).map(([_, v]) => v);
const document = toHtml_1.toHtmlDocument(pixels, 600, 800);
fs_1.default.writeFileSync('day-24/map.html', document);
console.log({
    bugCount
});
