...
const findSleigh = (x: number, y: number): { x: number, y: number } => {
    const ne = runProgramForCoordinate(x + 99, y)
    const sw = runProgramForCoordinate(x, y + 99)

    if(ne && sw) return { x, y }
    else if(ne) return findSleigh(x + 1, y)
    return findSleigh(x, y + 1)
}

const { x, y } = findSleigh(0, 0)
